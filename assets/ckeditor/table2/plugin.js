...
CKEDITOR.plugins.add( 'table2', {
	...
	init: function( editor ) {
		...
		editor.addCommand( 'table2', new CKEDITOR.dialogCommand( 'table2', {
			context: 'table2',
			allowedContent: 'table{width,height,border-collapse,border-spacing}[align,border,cellpadding,summary];' +
				'caption tbody thead tfoot;' +
				'th td tr[scope];' +
				'td{border*,background-color,vertical-align,width,height,padding}[colspan,rowspan];' +
				( editor.plugins.dialogadvtab ? 'table2' + editor.plugins.dialogadvtab.allowedContent() : '' ),
			requiredContent: 'table2',
		...
		function createDef( def ) {
			return CKEDITOR.tools.extend( def || {}, {
				...
				refresh: function( editor, path ) {
					this.setState( path.contains( 'table2', 1 ) ? CKEDITOR.TRISTATE_OFF : CKEDITOR.TRISTATE_DISABLED );
				}
			} );
		}
		...
		editor.ui.addButton && editor.ui.addButton( 'Table2', {
			label: lang.toolbar,
			command: 'table2',
			toolbar: 'insert,30'
		} );

		CKEDITOR.dialog.add( 'table2', this.path + 'dialogs/table2.js' );
		CKEDITOR.dialog.add( 'tableProperties', this.path + 'dialogs/table2.js' );
