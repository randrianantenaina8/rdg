		...
		return {
			...
			onLoad: function() {
				...
				if ( styles ) {
					styles.on( 'change', function() {
						...
						var cellSpace = this.getStyle( 'border-spacing', '' ),
							txtCellSpace = dialog.getContentElement( 'info', 'txtCellSpace' );

							txtCellSpace && txtCellSpace.setValue( cellSpace, true );

						var cellPadding = this.getStyle( 'padding', '' ),
							txtCellPadding = dialog.getContentElement( 'info', 'txtCellPadding' );

							txtCellPadding && txtCellPadding.setValue( cellPadding, true );
					} );
				}
			},
			...
			onShow: function() {
				...
				if ( command == 'tableProperties' ) {
					...
					if ( selected && selected.is( 'table2' ) )
						table = selected;
						...
						table = editor.elementPath( ranges[ 0 ].getCommonAncestor( true ) ).contains( 'table2', 1 );
					}
				...
				elements: [ {
					...
					},
					{
						...
						},
						{
							type: 'text',
							id: 'txtCellSpace',
							requiredContent: 'table{border-spacing}',
							controlStyle: 'width:3em',
							label: editor.lang.table.cellSpace,
							'default': '',
							validate: CKEDITOR.dialog.validate.cssLength( editor.lang.table.invalidCellSpacing.replace( '%1', editor.lang.table.cellSpace ) ),
							onChange: function() {
								var styles = this.getDialog().getContentElement('advanced', 'advStyles' );
							...
							setup: function( selectedTable ) {
								this.setValue( selectedTable.getStyle( 'cellSpacing' ) || '' );
							},
							commit: commitValue
						...
						},
						{
							...
							onChange: function() {
								...
								styles && styles.updateStyle( 'padding', this.getValue()+"px" );
							},
							setup: function( selectedTable ) {
			...
			dialogadvtab && dialogadvtab.createAdvancedTab( editor, null, 'table2' )
		] };
	}

	CKEDITOR.dialog.add( 'table2', function( editor ) {
		return tableDialog( editor, 'table2' );
	} );
