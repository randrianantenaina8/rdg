...
( function() {
	...
	var template = '<img alt="" src="" title="" border="" marginV="" marginH="" border-color="" style="" link="" />',
		templateBlock = new CKEDITOR.template(
			'<figure class="{captionedClass}">' +
				template +
				'<figcaption class="img_figcaption">{captionPlaceholder}</figcaption>' 
				+ '<p class="img_copyright">copyright</p>' +
			'</figure>' ),
	...
	function widgetDef( editor ) {
		...
		return {
			...
			styleableElements: 'img figure p small',
			...
			editables: {
				caption: {
					selector: '.img_figcaption',
					allowedContent: 'br em strong sub sup u s; a[!href,target]'
				},
				copyright: {
					selector: '.img_copyright',
					allowedContent: '&copy; br em strong sub sup u s; a[!href,target]'
				}
			},
			...
			data: function() {
				...
				this.parts.image.setAttributes( {
					...
					marginV: this.data.marginV,
					marginH: this.data.marginH,
					link: this.data.link,
					'border-color': this.data.borderColor,
					style: "border: " + this.data.border + (this.data.border&&"px solid ") + this.data.borderColor + ";margin: " + (this.data.marginV ? this.data.marginV : "0" ) + "px "+ (this.data.marginH ? this.data.marginH : "0" )+ "px "
				} ); 

				console.log("border: " + this.data.border + (this.data.border&&"px solid ") + this.data.borderColor + ";margin: " + (this.data.marginV ? this.data.marginV : "0" ) + "px "+ (this.data.marginH ? this.data.marginH : "0" )+ "px "
				)
			...
			init: function() {
				var helpers = CKEDITOR.plugins.image2,
					...
					data = {
						...
						border: image.getAttribute( 'border' ) || '',
						borderColor: image.getAttribute( 'border-color' ) || '',
						marginV: image.getAttribute( 'marginV' ) || '',
						marginH: image.getAttribute( 'marginH' ) || '',
			...
			getTitle: function() {
				var labelTooltip = ( this.data.title || '' ) + ' ' + this.pathName;

				return this.editor.lang.widget.label.replace( /%1/, labelTooltip );
			},

			getBorder: function() {
				var labelBorder = ( this.data.border || '' ) + ' ' + this.pathName;

				return this.editor.lang.widget.label.replace( /%1/, labelBorder );
			},

			getBorderColor: function() {
				var labelBorderColor = ( this.data.borderColor || '' ) + ' ' + this.pathName;

				return this.editor.lang.widget.label.replace( /%1/, labelBorderColor );
			},

			getMarginV: function() {
				var labelMarginV = ( this.data.marginV || '' ) + ' ' + this.pathName;

				return this.editor.lang.widget.label.replace( /%1/, labelMarginV );
			},
			getMarginH: function() {
				var labelMarginH = ( this.data.marginH || '' ) + ' ' + this.pathName;

				return this.editor.lang.widget.label.replace( /%1/, labelMarginH );
			},

			getLink: function() {
				var labelLink = ( this.data.link || '' ) + ' ' + this.pathName;
				return this.editor.lang.widget.label.replace( /%1/, labelLink );
			},
		};
	}
				...
				hasCaption:	function( shift, oldValue, newValue ) {
					...
					if ( newValue ) {
						...
						var figure = CKEDITOR.dom.element.createFromHtml( templateBlock.output( {
							...
							copyright: editor.lang.image2.copyright
						} ), doc );
	...
	function getWidgetAllowedContent( editor ) {
		var alignClasses = editor.config.image2_alignClasses,
			rules = {
				...
				img: {
					attributes: '!src,alt,title,link,width,height,border,marginV,marginH,border-color,style'
				},
				figure: {
